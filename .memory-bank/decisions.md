...
2026-04-30 - 草稿历史先使用浏览器本地存储
- 决策：草稿历史当前保存在浏览器 `localStorage`，最多保留最近 5 条已校验草稿。
- 原因：这样能验证持久化体验，同时不增加后端写文件接口和删除确认复杂度。
- 影响：当前历史只对当前浏览器有效；如果需要跨会话文件管理或多设备同步，应另行设计服务端存储。

2026-04-30 - 外部草稿生成必须显式确认
- 决策：当 provider 状态显示会产生外部请求时，`Generate draft` 先打开确认区，确认后才调用 `/api/codex/draft`。
- 原因：真实 OpenAI 调用会发送本地 taste、routine、mood-rule 和 playlist 上下文，属于需要明确知情的外部传输。
- 影响：后续任何会发送本地上下文的集成都应复用这个状态检查和确认模式。
