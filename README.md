...
当前已完成到阶段 2.12：本地 Node.js API 已能读取用户上下文文件，Settings 页可生成草稿、展示 provider 状态、应用草稿前确认，并在浏览器本地保留最近 5 条草稿历史。
...
http://127.0.0.1:8787/api/codex/provider-status
...
Settings 页面会把已校验草稿保存到浏览器 `localStorage`，最多保留最近 5 条。刷新页面后历史仍然存在，点击历史条目可以把它设为当前草稿，再通过确认流程应用到播放器。

当前历史只保存在当前浏览器中，不会写入后端文件，也不会跨设备同步。
